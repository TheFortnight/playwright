...
test('1557. successfull order', async ({ page, browserName }) => {
  ...
  await test.step('Submit order and verify success toast', async () => {
    ...
    await expect(page.locator('body .toast-message')).toBeVisible({ timeout: 15000 });
    await expect(page.locator('body .toast-message')).toContainText('Ваш заказ принят в работу', {
      timeout: 15000,
    });
  });
});

...
test('1409. basket show on map live', async ({ page, browserName }) => {
  ...
  await test.step('Open the map and verify offer plates', async () => {
    await expect(page.locator('#map')).toBeVisible({ timeout: 15000 });
    await expect(page.locator('body')).toContainText(/₽|рядом/, { timeout: 30000 });
  });
});

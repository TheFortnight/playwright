...
function parseDistanceLabel(text) {
  const match = String(text || '').replace(/\s+/g, ' ').trim().match(/([\d.,]+)\s*(км|м)/i);
  ...
  return match[2].toLowerCase() === 'км' ? value * 1000 : value;
}

test('1400. basket order by distance live', async ({ page }) => {
  await test.step('Open subcatalog and add first product to basket', async () => {
  ...
  await test.step('Verify distances are sorted by center', async () => {
    await expect.poll(async () => {
      const values = (await page.locator('body .distance-label').allTextContents())
        .map(parseDistanceLabel)
